...
vector<Mat<float>> loadAllPupilsCoordinates(string databaseDirectory, vector<string> &filenames)
{
	...
	for (auto filename : filenames)
	{
		...
		fscanf(f, "%*s %*s %*s %*s %f %f %f %f", 
			&actualFileCoordinates[1][0], &actualFileCoordinates[1][1], &actualFileCoordinates[0][0], &actualFileCoordinates[0][1]);
...
vector<SimpleRect> getEyesBounds(Mat<float> facialFeaturesCoordinates)
{
	vector<SimpleRect> eyeBounds(2);

	eyeBounds[0].set((int)(facialFeaturesCoordinates[36][0]) - 5, 
				 	 (int)(facialFeaturesCoordinates[39][0]) + 5,
					 (int)(facialFeaturesCoordinates[37][1]) - 5,
					 (int)(facialFeaturesCoordinates[41][1]) + 5);

	eyeBounds[1].set((int)(facialFeaturesCoordinates[42][0]) - 5, 
				     (int)(facialFeaturesCoordinates[45][0]) + 5,
					 (int)(facialFeaturesCoordinates[43][1]) - 5,
					 (int)(facialFeaturesCoordinates[47][1]) + 5);
